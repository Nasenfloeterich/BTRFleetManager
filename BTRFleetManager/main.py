...
class SelectCommand(discord.ui.View):
    ...
    @discord.ui.button(label="Overview", style=discord.ButtonStyle.danger, emoji="📈")
...
@client.tree.command(name="classes", description="-", guild=GUILD_ID)
@app_commands.checks.has_role(ADMIN_ROLE_ID)
async def get_classes(interaction: discord.Interaction):
    ...
    for name in table_names:
        cursor.execute(f"""SELECT Grid_Core FROM "{name[0]}" """)
        s_c = cursor.fetchall()
        print(s_c)
        s_class.append(s_c)
    ...
    print(s_class)
    await interaction.response.send_message(s_class)
